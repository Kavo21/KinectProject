...
{
    /// <summary>
    /// StudentsStoryMarking.xaml 的互動邏輯
    /// </summary>
    public partial class StudentsStoryMarking : Window
    {
        ...
        /// 打開句子類別選單
        /// </summary>
        private SelectSentenceView selectSentenceView = new SelectSentenceView();
        ...
        private List<String> StorageEmotionWords;
        ...
        {
            //叫出句子選擇葉面
            selectSentenceView.Show();

            //註冊事件觸發後的方法
            selectSentenceView.SelectEvent += Select_Item_View_SelectEvent;
        ...
        private void SpeechRecognized(object sender, SpeechRecognizedEventArgs e)
        {
            ...
            if (e.Result.Confidence >= BasicMethod.ConfidenceThreshold)
            {
                ...
                if (e.Result.Semantics.Value.ToString().ToLower() == vocabulary.Give_StudentsSentence(posture_number).ToLower())
                {
                    ...
                    if (Select_Item.SetSentenceGameMode == true)
                    {
                        //新增語音辨識信心度到List
                    ...
                    //只是為了再次呼叫 檢查正確與否的方法，所以PostureWords 設為空值
        ...
        {
            //回傳句子進度
            txtDetection.Content = vocabulary.ReturnSentenceProgress(posture_number);
            ...
            //判斷是否顯示範例圖片
            if (Select_Item.SetSentenceGameMode != true && Select_Item.SetStudentsSentencesGameMode !=true)
            {
                //Set Example Image
        ...
        private void CompareWords(String Sentence)
        {
            ...
            StorageEmotionWords = new List<string>();
            ...
            txtCountWords.Visibility = Visibility.Visible;

            //顯示句子中有幾個資料庫包含的單字
            txtCountWords.Content = (String.Format("剩餘單字 : {0}", TaskWords.Count.ToString()));

            //把檢測到的單字回存給情感作為偵測
            StorageEmotionWords = TaskWords;
        }
        ...
        private void Success_Change_word(object sender, EventArgs e)
        {
            if(e != null)
            {
                ...
                if (TaskWords.Contains(TargetWords))
                {
                    ...
                    txtCountWords.Content = (String.Format("剩餘 : {0}", TaskWords.Count.ToString()));
                ...
                if (!Speech_Result == false && !Posture_Result == false && !Speech_Enable == false && TaskWords.Count == 0)
                {
                    ...
                    if (Select_Item.SetSentenceGameMode == true)
                    {
                        ...
                        FirebaseUpload.Upload_Rakn_Point(Students.StudentsName, RankData.Rank_Interactive, TotalTime);
        ...
        public void Show_Sentence()
        {
            ...
            //Hide the countwords
            txtCountWords.Visibility = Visibility.Hidden;

            //顯示句子 
            txtSentence.Text = vocabulary.GiveChineseSentence(posture_number);
        ...
        {
            RankData rankData = new RankData(RankData.Rank_Interactive);
            rankData.Show();
        }
